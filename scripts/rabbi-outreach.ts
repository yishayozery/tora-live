...
function buildWhatsApp(a: Args): string {
  ...
  return [
    ...
    "",
    hook + `בניתי פלטפורמה בשם TANA (${SITE}) שמרכזת שיעורי תורה אונליין לרבני המגזר הדתי-לאומי והחרדל״י. זה חינם לגמרי, ו${painPoint}.`,
    "",
...
function buildEmail(a: Args): string {
  return [
    `נושא: הצעה לשיתוף פעולה — פלטפורמת TANA לשיעורי תורה`,
    "",
    `שלום כבוד הרב ${a.name.replace(/^הרב\s+/, "")},`,
    "",
    a.hook ? `${a.hook}.` : "",
    "",
    `שמי ${FOUNDER_NAME}, ובניתי פלטפורמה חדשה בשם TANA שמטרתה לרכז את שיעורי התורה של רבני המגזר הדתי-לאומי והחרדל״י במקום אחד — נגיש, חינמי, ועם כלים שיעזרו לרב להגיע לתלמידים.`,
    "",
...
function buildPhoneScript(a: Args): string {
  return [
    ...
    "",
    `## פתיחה (15 שניות)`,
    `"שלום, שמי ${FOUNDER_NAME}. אני מתקשר להציע לכבוד הרב פלטפורמה חדשה בשם TANA — בחינם — לריכוז השיעורים שלו במקום אחד. האם זה זמן נוח ל-2 דקות?"`,
    "",
    `## ההסבר (45 שניות)`,
